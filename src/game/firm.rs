...
use crate::game::{
    actor::Actor, config::{deal_constants, GameConfig}, contract::Contract, deal::{
...
pub struct Firm {
    ...
    /// Standing consume demand (`desire_needs`) of the owner or workforce.
    /// Plan treats this as a self-sale and crafts may not spend it.
    pub household_demand: HashMap<usize, f64>,
...
impl Firm {
    ...
    pub fn new(id: usize, name: String, market: usize, location: Hex) -> Self {
        Self {
            ...
            household_demand: HashMap::new(),
            owner_household_size: 0.0,
        }
    }

    /// Standing dinner and consume shortfall for goods this shop makes.
    /// Desire buckets skip substitutes the shop does not produce so remaining
    /// sat lands on in-shop targets. Call before production and at market open.
    pub fn refresh_household_needs(&mut self, pops: &HashMap<usize, Pop>, factuals: &Factuals) {
        self.household_needs.clear();
        self.household_demand.clear();
        let outputs = self.produced_goods(factuals);
        if outputs.is_empty() {
            return;
        }
        let mut ids = Vec::new();
        if let Some(id) = self.owners.pop_id() {
            ids.push(id);
        }
        ...
        ids.dedup();
        for pop_id in ids {
            let Some(pop) = pops.get(&pop_id) else {
                continue;
            };
            for tier in &pop.desires {
                for desire in tier {
                    ...
                    for target in desire.ordered_targets() {
                        ...
                        if !outputs.contains(&target.good) || target.efficiency <= 0.0 {
                            continue;
                        }
                        let qty = remaining.min(desire.amount * target.cap) / target.efficiency;
                        if qty <= 0.0 {
                            continue;
                        }
                        self.add_owner_want(pop, target.good, qty);
    ...
    fn add_owner_want(&mut self, pop: &Pop, good: usize, qty: f64) {
        if qty <= 0.0 {
            return;
        }
        *self.household_demand.entry(good).or_insert(0.0) += qty;
        let have = pop
            .property
            .get(&good)
            .map(|row| row.quantity.max(0.0))
            .unwrap_or(0.0);
        let short = (qty - have).max(0.0);
        if short > 0.0 {
            *self.household_needs.entry(good).or_insert(0.0) += short;
        }
    }

    /// Goods this firm's running lines output (not Time).
    ...
    /// Pulls `want` from `quantity` then `held`. Used when the owner eats
    /// from the shop shelf.
    pub fn take_from_shelf(&mut self, good: usize, want: f64) -> f64 {
        if want <= 0.0 {
            return 0.0;
        }
        ...
        let from_qty = want.min(row.quantity.max(0.0));
        row.quantity -= from_qty;
        let rest = want - from_qty;
        let from_held = rest.min(row.held.max(0.0));
        row.held -= from_held;
        row.sync_reserve();
        from_qty + from_held
    }
    ...
    fn production_available(&self) -> HashMap<usize, f64> {
        self.property
            .iter()
            .map(|(&gid, row)| {
                let stock = row.production_stock();
                let dinner = if gid == TIME {
                    0.0
                } else {
                    self.dinner_qty(gid)
                };
                (gid, (stock - dinner).max(0.0))
            })
            .collect()
    }

    /// Standing dinner qty for `good`, falling back to today's shortfall.
    pub fn dinner_qty(&self, good: usize) -> f64 {
        self.household_demand
            .get(&good)
            .copied()
            .or_else(|| self.household_needs.get(&good).copied())
            .unwrap_or(0.0)
            .max(0.0)
    }

    /// Merged standing demand and shortfall, per good.
    fn dinner_map(&self) -> HashMap<usize, f64> {
        let mut map = self.household_demand.clone();
        for (&good, &qty) in &self.household_needs {
            let entry = map.entry(good).or_insert(0.0);
            *entry = entry.max(qty.max(0.0));
        }
        map
    }
    ...
    /// Run owner-dinner lines first, then input-feeding lines, then higher
    /// recipe AMV profit. Among dinner, Time-only garden lines go before
    /// crafts that consume those same dinner goods, cheaper Time first.
    fn production_run_order(&self, factuals: &Factuals, market: &Market) -> Vec<usize> {
        ...
        let process_of = |i: usize| factuals.processes.get(&self.production_line[i].process);
        let owner_need = |i: usize| {
            process_of(i).is_some_and(|process| {
                process.outputs.iter().any(|output| {
                    self.dinner_qty(output.good) > 0.0
                })
            })
        };
        let eats_dinner = |i: usize| {
            process_of(i).is_some_and(|process| {
                process.inputs.iter().any(|input| {
                    input.good != TIME
                        && !input.is_optional()
                        && !matches!(input.input_type, crate::game::process::InputType::Factor)
                        && self.dinner_qty(input.good) > 0.0
                })
            })
        };
        let feed = |i: usize| {
        ...
        let time_cost = |i: usize| {
            let line = &self.production_line[i];
            let time_in = process_of(i)
                .and_then(|process| {
                    process
                        .inputs
                        .iter()
                ...
                .unwrap_or(0.0);
            line.target.unwrap_or(0.0).max(0.0) * time_in
        };
        let profit = |i: usize| {
            process_of(i)
        ...
        order.sort_by(|&a, &b| {
            owner_need(b)
                .cmp(&owner_need(a))
                .then(eats_dinner(a).cmp(&eats_dinner(b)))
                .then(feed(b).cmp(&feed(a)))
                .then_with(|| {
                    if owner_need(a) && owner_need(b) {
                        time_cost(a)
                            .partial_cmp(&time_cost(b))
...
impl Firm {
    ...
    pub fn pay_profit_share_amv(
        ...
        for good in goods {
            ...
            self.record_placed(good, give, price);
...
impl DealMaker for Firm {
    ...
    fn evaluate(
        ...
        let needs_received = deal
            .goods_received(role)
            .any(|(good, _)| firm_need_room(self, good) > 0.0);
        let keep = firm_amv_percent_keep(self, deal, role, history);
        let verdict = evaluate_keep_ratio(
            role,
            keep,
        ...
        if needs_received && keep >= factuals.config.deal.firm_amv_need_keep {
            return DealResponse::Accept;
        }
        DealResponse::Reject
    }
    ...
    fn sell(
        ...
        let _ = (own_order, other_order);
        let Some(role) = deal.role_of(Actor::Firm(self.id)) else {
            return deal.clone();
        };
        firm_make_change(self, deal, role, history, factuals.config.deal.firm_amv_min_keep)
    }
    ...
    fn finalize(&mut self, deal: &ProposedDeal, history: &MarketHistory) {
        ...
        for (&good, _) in &deal.goods {
            ...
            if delta > 0.0 {
                ...
                let mut sold = -delta;
                let from_qty = sold.min(row.quantity.max(0.0));
                row.quantity -= from_qty;
                sold -= from_qty;
                if sold > 0.0 {
                    debug_assert!(
                        row.held + 1e-12 >= sold,
                        "held must cover leftover sale (firm {} good {} held {} need {})",
                        self.id,
                        good,
                        row.held,
                        sold
                    );
                    row.held = (row.held - sold).max(0.0);
                }
                row.sold += -delta;
                row.sold_amv += (-delta) * price;
            }
            debug_assert!(
                row.quantity >= 0.0,
                "quantity must be >= 0.0 (firm {} good {} qty {})",
                self.id,
                good,
                row.quantity
            );
...
fn firm_have(firm: &Firm, good: usize) -> f64 {
    firm.property
        .get(&good)
        .map(|row| row.quantity.max(0.0))
        .unwrap_or(0.0)
...
fn firm_need_want(firm: &Firm, good: usize) -> f64 {
    let use_target = firm
        .property
        .get(&good)
        .map(|row| row.use_target.max(0.0))
        .unwrap_or(0.0);
    let purchase = firm
        .property
        .get(&good)
        .map(|row| row.purchase_target.max(0.0))
        .unwrap_or(0.0);
    let household = firm.household_needs.get(&good).copied().unwrap_or(0.0).max(0.0);
    use_target + purchase + household
}

fn firm_need_room(firm: &Firm, good: usize) -> f64 {
    (firm_need_want(firm, good) - firm_have(firm, good)).max(0.0)
}

fn firm_desired_room(firm: &Firm, good: usize) -> f64 {
    let stock = firm
        .property
        .get(&good)
        .map(|row| row.stock_target.max(0.0))
        .unwrap_or(0.0);
    let want = firm_need_want(firm, good).max(stock);
    (want - firm_have(firm, good)).max(0.0) - firm_need_room(firm, good)
}

fn firm_growth_room(firm: &Firm, good: usize) -> f64 {
    firm.property
        .get(&good)
        .map(|row| row.growth_target.max(0.0))
        .unwrap_or(0.0)
}

fn firm_band_factor(penalty: f64, salability: f64) -> f64 {
    let quote = crate::game::config::salability_quote_factor(salability);
    1.0 - penalty * (1.0 - quote)
}

/// Unit AMV this firm uses in keep: standing quote (bid if receiving, ask
/// if giving) when `amv_target` is set, otherwise live market AMV.
fn firm_quote_price(firm: &Firm, good: usize, history: &MarketHistory, giving: bool) -> f64 {
    let market = history.price(good);
    ...
    if giving {
        row.ask_amv(row.amv_target)
    } else {
        row.bid_amv(row.amv_target)
    }
}

fn firm_received_amv(firm: &Firm, history: &MarketHistory, good: usize, qty: f64) -> f64 {
    debug_assert!(qty >= 0.0 && qty.is_finite(), "received qty must be >= 0.0");
    let price = firm_quote_price(firm, good, history, false);
    let sal = history.salability(good);
    let need = firm_need_room(firm, good);
    let desired = firm_desired_room(firm, good).max(0.0);
    let growth = firm_growth_room(firm, good).max(0.0);
    let save_f = firm_band_factor(deal_constants::POP_AMV_SAVE_PENALTY, sal);
    let extra_f = firm_band_factor(deal_constants::POP_AMV_UNNEEDED_PENALTY, sal);
    let unused_f = crate::game::config::salability_quote_factor(sal);
    let from_need = qty.min(need);
    let rest = qty - from_need;
    let from_desired = rest.min(desired);
    let rest = rest - from_desired;
    let from_growth = rest.min(growth);
    let from_unused = rest - from_growth;
    from_need * price
        + from_desired * price * save_f
        + from_growth * price * extra_f
        + from_unused * price * unused_f
}

fn firm_amv_percent_keep(
    firm: &Firm,
    deal: &ProposedDeal,
    role: DealRole,
    history: &MarketHistory,
    ...
    for (good, qty) in deal.goods_given(role) {
        given += qty * firm_quote_price(firm, good, history, true);
    }
    if given <= 0.0 {
        return f64::INFINITY;
    }
    ...
    for (good, qty) in deal.goods_received(role) {
        received += firm_received_amv(firm, history, good, qty);
    }
    received / given
}
...
fn firm_make_change(
    ...
    let mut out = deal.clone();
    loop {
        let keep = firm_amv_percent_keep(firm, &out, role, history);
        if keep <= 1.0 + 1e-9 {
            break;
        }
        let mut dropped = false;
        let received: Vec<(usize, f64)> = out.goods_received(role).collect();
        for (good, qty) in received {
            let need = firm_need_room(firm, good);
            let desired = firm_desired_room(firm, good);
            let growth = firm_growth_room(firm, good);
            let kept = need + desired + growth;
            let unused = (qty - kept).max(0.0);
            if unused < 1.0 {
                continue;
            }
            let signed = out.goods.get(&good).copied().unwrap_or(0.0);
            let new_abs = (qty - 1.0).max(0.0);
            if new_abs <= 0.0 {
                out.goods.remove(&good);
            } else {
                out.goods.insert(good, signed.signum() * new_abs);
            }
            let next = firm_amv_percent_keep(firm, &out, role, history);
            if next + 1e-9 < min_keep {
                out.goods.insert(good, signed);
                continue;
            }
            dropped = true;
            break;
...
mod firm {
    ...
    mod refresh_household_needs_should {
        use super::*;
        ...
        fn owner_pop(id: usize, desire: Desire) -> Pop {
            let mut pop = Pop {
                id,
                job: 0,
                property: HashMap::new(),
        ...
        #[test]
        fn in_shop_target_takes_the_full_bucket() {
            let farm = Process::new(1, "farm", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(1, 2.0, true));
            let mut factuals = make_factuals_with_process(farm);
            factuals.goods.insert(1, make_good(1, "grain", HashMap::new()));
            factuals.goods.insert(3, make_good(3, "bread", HashMap::new()));
            ...
            let food = Desire {
                ...
                target: vec![
                    ...
                    DesireTarget::new(3, DesireTargetType::Consume, 1.5),
                ],
                amount: 5.0,
                satisfaction: 0.0,
            ...
            let pop = owner_pop(1, food);
            let mut firm = Firm::new(1, "well".into(), 42, hexx::Hex::new(0, 0))
                .with_owner(Actor::Pop(1))
                .with_owner_liability();
            let mut line = empty_production_line(1);
            line.target = Some(2.0);
            firm.production_line.push(line);
            let pops = HashMap::from([(1, pop)]);

            firm.refresh_household_needs(&pops, &factuals);

            assert_eq!(firm.household_demand.get(&1).copied(), Some(5.0));
            assert_eq!(firm.household_needs.get(&1).copied(), Some(5.0));
            assert_eq!(firm.household_demand.get(&3).copied(), None);
        }

        #[test]
        fn off_shop_want_stays_on_household_needs() {
            let farm = Process::new(1, "farm", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(1, 2.0, true));
            let mut factuals = make_factuals_with_process(farm);
            ...
            let shiny = Desire {
                source: DesireSource::Species(0, 5),
                priority: 5,
                target: vec![DesireTarget::new(4, DesireTargetType::Consume, 1.0)],
                amount: 5.0,
                satisfaction: 0.0,
                category: Some("shiny".into()),
                effect: vec![],
                scalar: ScalingFactor::All(1.0),
                decay: 0.0,
            };
            let pop = owner_pop(1, shiny);
            let mut firm = Firm::new(1, "well".into(), 42, hexx::Hex::new(0, 0))
                .with_owner(Actor::Pop(1))
                .with_owner_liability();
            let mut line = empty_production_line(1);
            ...
            assert_eq!(firm.household_demand.get(&4).copied(), Some(5.0));
            assert_eq!(firm.household_needs.get(&4).copied(), Some(5.0));
        }
    }
    ...
    mod run_production_should {
        ...
        #[test]
        fn optional_booster_leaves_owner_dinner() {
            let grain = Process::new(1, "grain", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_input(ProcessInput::new(2, 0.5, true, InputType::Destroyed, true))
                .with_output(ProcessOutput::new(1, 6.0, true));
            let mut factuals = make_factuals_with_process(grain);
            factuals.goods.insert(1, make_good(1, "grain", HashMap::new()));
            factuals.goods.insert(2, make_good(2, "water", HashMap::new()));
            factuals.goods.insert(TIME, make_good(TIME, "time", HashMap::new()));
            factuals.config.firm.complexity_time_factor = 0.0;

            let mut firm = Firm::new(1, "farm".into(), 42, hexx::Hex::new(0, 0))
                .with_owner_liability();
            firm.household_needs.insert(2, 5.0);
            firm.property.insert(TIME, FirmPRow::new().with_quantity(4.0));
            firm.property.insert(2, FirmPRow::new().with_quantity(6.0));
            let mut grain_line = empty_production_line(1);
            grain_line.target = Some(8.0);
            firm.production_line.push(grain_line);

            let market = make_market_with_amvs(&[(1, 1.0), (2, 1.0), (TIME, 1.0)]);
            firm.run_production(&factuals, &market);

            assert_eq!(firm.production_line[0].last_iterations, 8.0);
            let water = firm.property[&2].quantity + firm.property[&2].held;
            assert_eq!(water, 5.0, "optional water ate dinner, left {water}");
        }

        #[test]
        fn owner_dinner_runs_before_specialty() {
            let gold = Process::new(1, "gold", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(4, 1.0, true));
            let forage = Process::new(2, "forage", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(7, 2.0, true));
            let mut factuals = make_factuals_with_process(gold);
            factuals.processes.insert(2, forage);
            factuals.goods.insert(4, make_good(4, "gold", HashMap::new()));
            factuals.goods.insert(7, make_good(7, "wood", HashMap::new()));
            factuals.goods.insert(TIME, make_good(TIME, "time", HashMap::new()));
            factuals.config.firm.complexity_time_factor = 0.0;

            let mut firm = Firm::new(1, "camp".into(), 42, hexx::Hex::new(0, 0))
                .with_owner_liability();
            firm.household_needs.insert(7, 5.0);
            firm.property.insert(TIME, FirmPRow::new().with_quantity(1.0));
            let mut gold_line = empty_production_line(1);
            gold_line.target = Some(5.0);
            let mut forage_line = empty_production_line(2);
            forage_line.target = Some(2.0);
            firm.production_line.push(gold_line);
            firm.production_line.push(forage_line);

            let market = make_market_with_amvs(&[(4, 10.0), (7, 1.0), (TIME, 1.0)]);
            firm.run_production(&factuals, &market);

            assert_eq!(firm.production_line[1].last_iterations, 2.0);
            assert_eq!(
                firm.production_line[0].last_iterations, 0.0,
                "specialty spent Time before dinner"
            );
        }

        #[test]
        fn garden_runs_before_a_craft_that_eats_dinner() {
            let bake = Process::new(1, "bake", 0)
            ...
            let forage = Process::new(2, "forage", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(7, 2.0, true));
            let mut factuals = make_factuals_with_process(bake);
            factuals.processes.insert(2, forage);
            factuals.goods.insert(1, make_good(1, "grain", HashMap::new()));
            factuals.goods.insert(3, make_good(3, "bread", HashMap::new()));
            factuals.goods.insert(7, make_good(7, "wood", HashMap::new()));
            ...
            let mut firm = Firm::new(1, "bakery".into(), 42, hexx::Hex::new(0, 0))
                .with_owner_liability();
            firm.household_needs.insert(1, 5.0);
            firm.household_needs.insert(3, 5.0);
            firm.household_needs.insert(7, 5.0);
            firm.property.insert(TIME, FirmPRow::new().with_quantity(1.0));
            firm.property.insert(1, FirmPRow::new().with_quantity(5.0));
            let mut bake_line = empty_production_line(1);
            bake_line.target = Some(5.0);
            let mut forage_line = empty_production_line(2);
            forage_line.target = Some(2.0);
            firm.production_line.push(bake_line);
            firm.production_line.push(forage_line);
    ...
    mod scale_amv_unit_should {
        ...
        fn multiplies_quotes_and_cost_basis() {
            ...
            assert!((row.placed_amv - 4.0).abs() < 1e-12);
    ...
    mod plan_should {
        ...
        fn leftover_buys_are_not_a_sell_miss() {
            ...
            firm.plan(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(got >= 4.0, "treated leftover buys as a miss, got {got}");
        }
        ...
        fn quiet_mid_clear_keeps_the_line_target() {
            ...
            // sell success 0.6 sits in the shrink..grow band; leftover matches
            // operations_cover (post-shop 20 = 4 planned * 5 days).
            plank.sold = 2.4;
            plank.produced = 4.0;
        ...
        #[test]
        fn output_only_line_fences_five_days_of_output() {
            let process = Process::new(1, "farm", 0)
            ...
            factuals.goods.insert(
                TIME,
                make_good(TIME, "time", HashMap::new()),
            );
            factuals.goods.get_mut(&TIME).unwrap().tags.insert(GoodTag::Untradeable);
            factuals.goods.insert(20, make_good(20, "grain", HashMap::new()));
            factuals.config.firm.planning_lerp_rate = 1.0;
            let history = make_history(&[(TIME, 1.0), (20, 5.0)]);
            let mut firm = Firm::new(1, "farm".into(), 42, hexx::Hex::new(0, 0));
            let mut line = empty_production_line(1);
            line.target = Some(4.0);
            line.inputs = vec![TIME];
            firm.production_line.push(line);
            firm.property.insert(TIME, FirmPRow::new().with_quantity(4.0));
            firm.property.insert(20, FirmPRow::new().with_quantity(0.0));
            firm.plan(&factuals, &history);
        ...
        #[test]
        fn empty_output_asks_five_days_of_input() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.property.get_mut(&20).unwrap().quantity = 0.0;
            firm.plan(&factuals, &history);
        ...
        #[test]
        fn perishable_output_shrinks_the_hold() {
            let (mut factuals, history) = miller_world();
            factuals.goods.get_mut(&20).unwrap().decay_rate = 0.4;
            let mut firm = miller_firm(4.0);
            firm.property.get_mut(&20).unwrap().quantity = 0.0;
            firm.plan(&factuals, &history);
            // 4 planks/day * 5 * 0.6 hold days
            assert!((firm.property[&20].stock_target - 12.0).abs() < 1e-12);
        }

        #[test]
        fn leftover_profit_stops_at_operations_cover() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.plan(&factuals, &history);
            firm.property.get_mut(&20).unwrap().quantity = 20.0;
            firm.property.get_mut(&20).unwrap().sell_target = 0.0;
            firm.property.get_mut(&10).unwrap().quantity = 0.0;
            firm.property.get_mut(&5).unwrap().quantity = 0.0;
            assert_eq!(firm.leftover_profit_amv(&history, &factuals), 0.0);
            firm.property.get_mut(&20).unwrap().quantity = 30.0;
            assert!((firm.leftover_profit_amv(&history, &factuals) - 50.0).abs() < 1e-12);
        }

        #[test]
        fn in_kind_placement_counts_toward_sell_success_up_to_the_fence() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.sell_target = 10.0;
            plank.sold = 0.0;
            plank.placed = 8.0;
            plank.placed_amv = 40.0;
            plank.stock_target = 10.0;
            plank.produced = 10.0;
            plank.average_cost = 5.0;
            firm.record_keeping(&factuals, &history);
            assert!((firm.records.sell_success - 0.8).abs() < 1e-12);
            assert!((firm.records.profit_ratio - 1.0).abs() < 1e-12);
        }

        #[test]
        fn placement_above_the_fence_is_capped() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.sell_target = 10.0;
            plank.sold = 0.0;
            plank.placed = 50.0;
            plank.placed_amv = 250.0;
            plank.stock_target = 4.0;
            plank.produced = 10.0;
            plank.average_cost = 5.0;
            firm.record_keeping(&factuals, &history);
            assert!((firm.records.sell_success - 0.4).abs() < 1e-12);
        }
        ...
        fn unequal_profit_shifts_output_to_the_better_line() {
            ...
            plank.sold = 12.0;
            plank.produced = 20.0;
            // post-shop 100 = 20 planned * 5 days operations cover
            plank.quantity = 120.0;
            plank.amv_target = 5.0;
            firm.plan(&factuals, &history);
        ...
        #[test]
        fn quote_orbits_market_amv_not_the_recipe_floor() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.property.get_mut(&20).unwrap().amv_target = 2.0;
            firm.plan(&factuals, &history);
            let quote = firm.property[&20].amv_target;
            let market = history.price(20);
            assert_eq!(firm.property[&20].amv_bound, FirmAmvBound::Minimum(2.0));
            assert!(quote >= market * 0.9 - 1e-9, "got {quote} market {market}");
            assert!(quote <= market * 1.1 + 1e-9, "got {quote} market {market}");
            assert!((quote - 2.0).abs() > 0.5, "still pinned to floor {quote}");
        }

        #[test]
        fn sold_through_raises_quote_even_with_rejects() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.amv_target = 5.0;
            plank.sell_fills = 4.0;
            plank.sell_rejects = 12.0;
            plank.quantity = 24.0;
            firm.plan(&factuals, &history);
            let quote = firm.property[&20].amv_target;
            assert!(quote > 5.0, "got {quote}");
            assert_eq!(firm.production_line[0].target, Some(4.0));
        }

        #[test]
        fn failed_meetings_cut_quote_when_the_shelf_does_not_clear() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 0.0, 4.0, 8.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.amv_target = 5.5;
            plank.quantity = 8.0;
            plank.sell_fills = 0.0;
            plank.sell_rejects = 6.0;
            firm.plan(&factuals, &history);
            let quote = firm.property[&20].amv_target;
            assert!(quote < 5.5, "got {quote}");
        }
        ...
        #[test]
        fn strong_sales_at_cost_raises_quote_not_quota() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            let plank = firm.property.get_mut(&20).unwrap();
        ...
        #[test]
        fn a_good_sales_day_holds_quota_through_a_miss() {
            let (mut factuals, history) = miller_world();
            factuals.config.firm.planning_lerp_rate = 0.15;
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            firm.property.get_mut(&20).unwrap().amv_target = 5.0;
            ...
            assert!((firm.property[&20].sold_avg - 4.0).abs() < 1e-12);
            firm.clear_day_flows();
            mark_hit(&mut firm, 0.0, 5.0, 5.0);
            firm.property.get_mut(&20).unwrap().amv_target = 5.0;
            firm.record_keeping(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(got >= 4.5, "one miss cut quota, got {got}");
        }

        #[test]
        fn placed_without_market_sales_does_not_grow_quota() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 0.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            ...
            firm.plan(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(got <= 4.0, "grew from remainder placement, got {got}");
        }

        #[test]
        fn owner_need_without_sales_does_not_cut_quota() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 0.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            firm.household_needs.insert(20, 5.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.amv_target = 5.0;
            plank.average_cost = 5.0;
            firm.plan(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(got >= 4.0, "cut owner dinner, got {got}");
        }

        #[test]
        fn owner_need_above_output_raises_quota() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 0.0, 4.0, 20.0);
            firm.production_line[0].aim = 4.0;
            firm.household_needs.insert(20, 5.0);
            let plank = firm.property.get_mut(&20).unwrap();
            plank.amv_target = 5.0;
            plank.average_cost = 5.0;
            plank.sell_no_proposal = 10.0;
            firm.plan(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(got > 4.0, "did not raise for owner dinner, got {got}");
        }

        #[test]
        fn craft_quota_leaves_owner_dinner_inputs() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.household_demand.insert(10, 5.0);
            firm.plan(&factuals, &history);
            let got = firm.production_line[0].target.unwrap();
            assert!(
                got <= 2.0,
                "craft spent owner wood, mill {got}"
            );
        }

        #[test]
        fn craft_may_use_surplus_after_dinner() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.household_demand.insert(10, 1.0);
            firm.plan(&factuals, &history);
            assert_eq!(firm.production_line[0].target, Some(4.0));
        }

        #[test]
        fn time_starved_peer_blocks_quota_growth() {
            let (mut factuals, history) = miller_world();
            let garden = Process::new(2, "garden", 0)
                .with_input(ProcessInput::new(TIME, 0.5, true, InputType::Destroyed, false))
                .with_output(ProcessOutput::new(7, 2.0, true));
            factuals = factuals.with_process(garden);
            factuals.goods.insert(7, make_good(7, "herb", HashMap::new()));
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            let plank = firm.property.get_mut(&20).unwrap();
            plank.amv_target = 5.0;
            plank.quantity = 0.0;
            let mut garden_line = empty_production_line(2);
            garden_line.target = Some(2.0);
            garden_line.last_missing_goods = vec![TIME];
            firm.production_line.push(garden_line);
            firm.property.insert(7, FirmPRow::new());
            firm.plan(&factuals, &history);
            assert_eq!(
                firm.production_line[0].target,
                Some(4.0),
                "grew while a peer missed Time"
            );
        }

        #[test]
        fn quota_steps_up_a_full_iteration_on_strong_sales() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            ...
            assert_eq!(firm.production_line[0].aim, 4.0);
        }

        #[test]
        fn quota_steps_down_a_full_iteration_on_a_miss() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 0.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
        ...
        #[test]
        fn quiet_day_lerps_aim_toward_throughput() {
            let (mut factuals, history) = miller_world();
            factuals.config.firm.planning_lerp_rate = 0.15;
            let mut firm = miller_firm(4.0);
            let line = &mut firm.production_line[0];
            ...
            plank.amv_target = 5.0;
            firm.plan(&factuals, &history);
            let aim = firm.production_line[0].aim;
            assert!((aim - 7.4).abs() < 1e-12, "got {aim}");
            let got = firm.production_line[0].target.unwrap();
            assert!(got > 4.0 && got < 8.0, "got {got}");
        }

        #[test]
        fn growth_target_is_the_expansion_gap_on_a_hit() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            mark_hit(&mut firm, 4.0, 4.0, 4.0);
            firm.production_line[0].aim = 4.0;
            ...
            firm.plan(&factuals, &history);
            assert_eq!(firm.production_line[0].target, Some(5.0));
            assert_eq!(firm.property[&20].growth_target, 1.0);
        }

        #[test]
        fn growth_target_is_zero_when_the_line_misses() {
            let (factuals, history) = miller_world();
            let mut firm = miller_firm(4.0);
            firm.production_line[0].aim = 4.0;
            firm.production_line[0].last_success_rate = 0.0;
    ...
    mod create_orders_should {
        ...
        fn miller_sells_output_and_buys_input_shortfall() {
            ...
            assert_eq!(orders[0].priority, 11.0);
        ...
        fn merchant_buys_beyond_stock_and_sells_current_sellable() {
            ...
            assert_eq!(orders[0].priority, 10.0);
        ...
        fn producer_dumps_only_excess_above_stock() {
            ...
            assert_eq!(orders[0].priority, 5.0);
        }
    ...
    mod deal_should {
        ...
        #[test]
        fn evaluate_uses_own_quote_as_the_ask() {
            let mut firm = empty_firm();
            firm.property.insert(20, FirmPRow::new().with_amv_target(2.0));
            let factuals = make_factuals_goods(&[1, 20]);
            let history = make_history(&[(1, 1.0, 1.0), (20, 1.0, 1.0)]);
            ...
            let deal = ProposedDeal::new(Actor::Pop(1), Actor::Firm(7))
                .with_good(20, -4.0)
                .with_good(1, 4.0);
            // given 4*quote 2 = 8, received 4*1 = 4, keep 0.5
            assert_eq!(
                firm.evaluate(&deal, &own, &other, &history, &factuals),
                DealResponse::Accept
            );
            firm.property.get_mut(&20).unwrap().amv_target = 3.0;
            // given 12, received 4, keep 0.33
            assert_eq!(
                firm.evaluate(&deal, &own, &other, &history, &factuals),
                DealResponse::Reject
            );
        ...
        #[test]
        fn evaluate_household_need_takes_low_salability_tender() {
            let mut firm = empty_firm();
            firm.household_needs.insert(11, 4.0);
            let factuals = make_factuals_goods(&[11, 20]);
            let history = make_history(&[(11, 1.0, 0.1), (20, 1.0, 0.4)]);
            let own = MarketOrder::offer_order(
                Actor::Firm(7),
                20,
                -4.0,
        ...
        #[test]
        fn sell_returns_unused_tender_as_make_change() {
            let mut firm = empty_firm();
            firm.household_needs.insert(11, 4.0);
            let factuals = make_factuals_goods(&[11, 20]);
            let history = make_history(&[(11, 1.0, 0.1), (20, 1.0, 1.0)]);
            let own = MarketOrder::offer_order(
                Actor::Firm(7),
                20,
                -4.0,
            ...
            let changed = firm.sell(&deal, &own, &other, &history, &factuals);
            assert_eq!(changed.goods.get(&11).copied().unwrap_or(0.0), 4.0);
        }
