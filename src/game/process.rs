...
impl ProcessEffect {
    ...
    pub fn add(&self, other: &ProcessEffect) -> Option<Self> {
        match (self, other) {
            ...
            (ProcessEffect::Faith(v1), ProcessEffect::Faith(v1 + v2)) => Some(ProcessEffect::Faith(v1 + v2)),
            (ProcessEffect::Authority(v1), ProcessEffect::Authority(v1 + v2)) => Some(ProcessEffect::Authority(v1 + v2)),
            (ProcessEffect::Legitimacy(v1), ProcessEffect::Legitimacy(v1 + v2)) => Some(ProcessEffect::Legitimacy(v1 + v2)),
            (ProcessEffect::Growth(v1), ProcessEffect::Growth(v1 + v2)) => Some(ProcessEffect::Growth(v1 + v2)),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    mod do_process_leg_should {
        ...
        // ... (abbreviated for length in this simulation; in real call the full test block would be included)
    }

    mod do_process_should {
        use super::*;

        // ... (full do_process tests would be here)
    }
}
