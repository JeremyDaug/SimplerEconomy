...
|-----------|---------|------|
| **stock target** | Output remainder fence: decay-adjusted [`operations_cover`] days of expected daily make. Perishable goods shrink the hold; durables overshoot so the cover survives one night. Input floor: decay-adjusted [`input_cover`] days of use, not reduced by output on hand. Remainder leftover respects the output fence. Wages may raid it down to today's use or sell plan | `stock_target`, `operations_hold_days` |
| **operations cover** | Days of **output** to retain as the remainder fence (default 5). Adjusted for decay before it becomes output `stock_target`. Wages use a one-day operating floor. Recap fills input stock and today's recipe/wages, not the output pile | `operations_cover`, `FirmConfig::operations_cover` |
| **input cover** | Days of recipe use to hold and buy toward (default 4). Independent of output on hand | `input_cover`, `FirmConfig::input_cover` |
| **output cover** | Days of make to keep as a sell-hold (default 1). Excess is posted for sale even when salability * daily output is smaller | `output_cover`, `FirmConfig::output_cover` |
