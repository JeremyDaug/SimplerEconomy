...
Init firms are read to attach each pop's specialty, then dropped.
Default CSV stem is `pop_prices`.

Each morning: `start_day` Time, reserve today's consume, then
`Pop::run_household_work` (subsistence basket, then the specialty).
Then market / consume / decay / pop `record_keeping`. The day report
includes `Pop production` (recipe, did, want, flows). A day-report table
with no rows is omitted. Opening stock is still one day of the specialty.
There is no morning top-up.
...
starts there. Owners eat from the pop bag. Goods the shop makes are
not posted as leftover-buy demand. Owner shortfall does not raise quota.
...
Living roster: five pops (bread, gold, second grain, second well, cabins).
The grain, water, and wood firms are still in `data/init/firms.toml` with
no matching pop. 1 household each.
...
Each firm is the matching pop's remainder owner-operator, one line,
`target` 8 on grain/water/wood, 5 on bread/gold,
...
1. `Pop::start_day` (Time grant). Tester morning specialty grant is 0
   (`DAILY_ENDOWMENT` 0, `DAILY_OUTPUT` 0). Zero `income_amv`, reservations,
   then `Pop::run_household_work` (basket, then that pop's specialty). Firm
   `clear_day_flows`.
...
3. `Firm::run_production` on **already-loaded** factuals. Do not reload
   `processes.toml`. A line starting from 0 snaps to 1. Outputs go to
   `held`; later lines may spend `held` after on-hand stock. Run order is
   input-feeding, then higher recipe AMV profit.
4. `Market::run_market_day` (may sell `held`). Time is untradeable
   ...
   close is recorded.
5. `Pop::consume` from the pop bag; `update_sentiments` (market-day
   history), then `decay_goods`. Firm
